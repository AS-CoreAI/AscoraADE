...
import { getBuiltinModels } from '@/lib/omnirouteProviderModels.generated'
...
export function ProvidersPage({ language, refreshKey }: { language: AppLanguage; refreshKey: number }): JSX.Element {
  ...
  const filteredEntries = useMemo(() => entries.filter((entry) => {
    ...
    if (modelNeedle) {
      const runtimeMatch = (modelIndex.get(entry.id) ?? []).some((model) => model.toLowerCase().includes(modelNeedle))
      const builtinMatch = getBuiltinModels(entry.id).some((model) => model.id.toLowerCase().includes(modelNeedle) || model.name.toLowerCase().includes(modelNeedle))
      if (!runtimeMatch && !builtinMatch) return false
    }
    return true
  ...
  const builtinModels = useMemo(() => (selectedEntry ? getBuiltinModels(selectedEntry.id) : []), [selectedEntry?.id])
  const liveModels = selectedModels.map((model) => ({ id: omniText(model.id), name: omniText(model.name ?? model.displayName ?? model.id) }))
  const modelRows = liveModels.length > 0 ? liveModels : builtinModels
  const showingBuiltinModels = liveModels.length === 0 && builtinModels.length > 0
  ...
  return <div className="omni-page omni-providers-page">
    ...
    {resource.loading ? <Loading /> : <div className={`omni-provider-browser${selectedEntry ? ' has-detail' : ''}`}>
      ...
      {selectedEntry && <aside className="omni-provider-detail">
        ...
        <section className="omni-provider-detail-section"><div className="omni-panel-title"><span>{l(language, 'Модели', 'Models')}</span><Badge>{modelRows.length}</Badge>{showingBuiltinModels && <Badge tone="blue">{l(language, 'Встроенные', 'Built-in')}</Badge>}{selectedConnections[0] && <button type="button" className="btn" onClick={() => void perform(`sync:${omniId(selectedConnections[0])}`, () => omniRequest('POST', `/api/providers/${encodeURIComponent(omniId(selectedConnections[0]))}/sync-models?mode=sync`), l(language, 'Модели синхронизированы.', 'Models synced.'))}><Icon name="refresh" size={12} />Sync</button>}</div>{modelsLoading ? <Loading /> : modelRows.length === 0 ? <div className="omni-provider-detail-empty">{l(language, 'Модели не найдены.', 'No models found.')}</div> : <div className="omni-provider-model-list">{modelRows.slice(0, 200).map((model) => <span key={model.id} title={model.name || model.id}>{model.id}</span>)}</div>}</section>
      </aside>}
    </div>}
