...
### Fixed
- **DeepSeek WProvider auth no longer closes too early** — DeepSeek is now considered ready only after both its persisted token and the prompt composer are available. The visible sign-in window stays open while DeepSeek is still on sign-in/verifying/loading states, preventing the "Checking DeepSeek..." flow from closing before the user can enter data.
